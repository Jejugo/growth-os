 ...
 */
/**
 * Extraída do `task()` pra poder ser chamada direto (sem Trigger.dev configurado, em dev) — a
 * SDK não expõe um `.run()` síncrono no objeto `Task`, só `.trigger()`/`.triggerAndWait()`.
 */
export async function runAutoApproveValidationPosts(payload: { productId: string }) {
...
export const autoApproveValidationPostsTask = task({
  ...
  maxDuration: 60,
  run: runAutoApproveValidationPosts,
})
